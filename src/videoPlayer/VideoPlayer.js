...
const VideoPlayer = (props) => {
  ...
  if (!props.video || !props.type || !props.videoWidth || !props.videoHeight) {
    return (
      <div className="missing">Properties are missing that are required for this component to work properly.</div>
    )
  }
  ...
  return (
    <div className="main">
      <div
        ...
        style={{ width: props.videoWidth, height: props.videoHeight }}
      >
        {settingsVisible && !menu ?
          <div id="settings" className="video__container--settings">
            <div>
              <p>Autoplay</p>
              <div className="switch" onClick={() => { setAutoplay(!autoplay); }}>
                <input type="checkbox" checked={autoplay} onChange={e => {}} />
                <span className="slider round"></span>
              </div>
            </div>
            <div>
              <p>Playback Speed</p>
              <p onClick={() => { showMenu('speed'); }}><span>{speed === 1 ? 'Normal' : speed}</span> <img type="image" src={right} height={12} width={20} alt="speed" /></p>
            </div>
            <div>
              <p>Quality</p>
              <p onClick={() => { showMenu('quality'); }}><span>{quality}</span> <img type="image" src={right} height={12} width={20} alt="quality" /></p>
            </div>
          </div>
        : null}
        {settingsVisible && menu && menu === 'speed' ?
          <div id="speed" className="video__container--speed">
            <div onClick={() => { showMenu(null); }}>
              <p><img type="image" src={left} height={12} width={20} alt="speed" /> <span>Playback Speed</span></p>
            </div>
            <div>
              <p onClick={() => { controlSpeed(0.25); }}>0.25</p>
              <p onClick={() => { controlSpeed(0.5); }}>0.5</p>
              <p onClick={() => { controlSpeed(0.75); }}>0.75</p>
              <p onClick={() => { controlSpeed(1); }}>Normal</p>
              <p onClick={() => { controlSpeed(1.25); }}>1.25</p>
              <p onClick={() => { controlSpeed(1.5); }}>1.5</p>
              <p onClick={() => { controlSpeed(1.75); }}>1.75</p>
              <p onClick={() => { controlSpeed(2); }}>2</p>
            </div>
          </div>
        : null}
        {settingsVisible && menu && menu === 'quality' ?
          <div id="quality" className="video__container--quality">
            <div onClick={() => { showMenu(null); }}>
              <p><img type="image" src={left} height={12} width={20} alt="speed" /> <span>Quality</span></p>
            </div>
            <div>
              <p onClick={() => { controlQuality('720p'); }}>720p</p>
              <p onClick={() => { controlQuality('480p'); }}>480p</p>
              <p onClick={() => { controlQuality('360p'); }}>360p</p>
              <p onClick={() => { controlQuality('240p'); }}>240p</p>
              <p onClick={() => { controlQuality('144p'); }}>144p</p>
              <p onClick={() => { controlQuality('Auto'); }}>Auto</p>
            </div>
          </div>
        : null}
        {props.video && props.type ?
          <video id="video_player">
            <source src={props.video} type={props.type} />
          </video>
        : null}
        <div className="video__container--timedisplay">
          <span id="video_time" style={{ background: (props.trackColor ? props.trackColor : 'rgb(248, 14, 1)') }}></span>
        </div>
        <input type="range" onChange={(event) => { videoControl(event); }} step="0.01" min="0" max="10" value={videoPosition} />
        <div id="video_controls" className='video__container--controls'>
          <div className='video__container--controls-left'>
            {playing === true ?
              <img type="image" src={pause} onClick={() => { pauseVideo(); }} alt="pause" /> :
              <img type="image" src={play} onClick={() => { playVideo(); }} alt="play" />
            }
            <div className="video__container--controls-left-volume" onMouseEnter={() => { setVolumeVisible(true); }} onMouseLeave={() => { setVolumeVisible(false); }}>
              {muted ? <img src={mute} alt="vol" onClick={() => { setVolumeControl(false); }} /> : <img src={volume} alt="vol" onClick={() => { setVolumeControl(true); }} />}
              {volumeVisible ? <input type="range" onChange={(event) => { changeVolume(event); }} step="0.01" min="0" max="1" value={level} /> : null}
            </div>
            <span>{minutes}:{seconds < 10 ? ("0" + seconds) : seconds} / {totalMinutes}:{totalSeconds}</span>
          </div>
          <div className='video__container--controls-right'>
            <img id="settingsicon" src={settings} alt="settings" onClick={() => { showSettings(); }} />
            <img src={defaultView} alt="defaultView" onClick={() => { defaultVideo(); }} />
            {view === 'expanded' ? <img src={normalView} alt="normalView" onClick={() => { normalVideo(); }} /> : <img src={expandView} alt="expandView" onClick={() => { expandVideo(); }} />}
          </div>
        </div>
      </div>
    </div>
